...
class GetHostROView extends \CController
{
    ...
    protected function doAction(): void
    {
        ...
        $hostids_raw = $_REQUEST['hostids'] ?? $_REQUEST['hostid'] ?? null;

        $hostids = [];
        if ($hostids_raw !== null) {
            if (is_array($hostids_raw)) {
                $hostids = array_map('intval', $hostids_raw);
            } else {
                $hostids = array_map('intval', explode(',', $hostids_raw));
            }
            $hostids = array_filter(array_unique($hostids));
        }

        if ($export !== null && !empty($hostids)) {
            // Get severity colors
    ...
    /**
     * CSV EXPORT - 3 Sections (Sheet1: General/Tags/Macros, Sheet2: Items, Sheet3: Triggers)
     */
    private function exportToCSV(array $allHostsData, array $severities): void
    {
        ...
        $filename = 'hosts_export_' . date('Y-m-d_Hi') . '.csv';
    ...
    /**
     * HTML EXPORT - Original Style (Multi-Host)
     */
    private function exportToHTML(array $allHostsData, array $severities): void
    {
        ...
        $filename = 'hosts_export_' . date('Y-m-d_Hi') . '.html';
...
<head>
    <meta charset="UTF-8">
    <title>Multi-Host Configuration Export</title>
    <style>
        body {
...
<body>
    <div class="page">
        <div class="header">
            <h1>Multi-Host Configuration Export</h1>
...
<?php endforeach; ?>
    ...
    private function exportToJSON(array $allHostsData): void
    {
        ...
        $filename = 'hosts_export_' . date('Y-m-d_Hi') . '.json';
